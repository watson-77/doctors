...
function doctors_filter_enqueue_scripts()
{
    ...
    if (is_post_type_archive('doctors') || is_tax('City') || is_tax('Specialization')) {
        ...
        wp_enqueue_script(
            'doctors-filter',
            get_stylesheet_directory_uri() . '/js/adl-core.js',
            array('jquery'),
...
add_action('wp_footer', 'add_apicoder_div_to_footer');

function apicoder_doctors_specialization($doctor_id)
{
    $spec_terms = wp_get_object_terms($doctor_id, 'Specialization');
    if (!empty($spec_terms) || !is_wp_error($spec_terms)) {
        $spec_links = [];
        foreach ($spec_terms as $spec) {
            $spec_links[] = '<a class="text-decoration-none" href="' . get_term_link($spec) . '"><span class="text-muted">' . __('' . $spec->name . '', 'apicoder') . '</span></a>';
        }
        $specialisation_list = '<p>Специализация: ' . implode(', ', $spec_links) . '.</p>';
    }
    print $specialisation_list;
}
function apicoder_doctor_city($doctor_id)
{
    $city_terms = wp_get_object_terms($doctor_id, 'City');
    if (!empty($city_terms) && !is_wp_error($city_terms)) {
        $city_links = [];
        foreach ($city_terms as $city) {
            $city_links[] = '<a class="text-decoration-none me-1" href="' . get_term_link($city) . '"><span class="text-muted">' . $city->name . '</span></a>';
        }
        $city_link = implode(', ', $city_links) . '.';
    }
    print $city_link;
}

function apicoder_excerpt()
{
    if (has_excerpt()) {
        // Если есть ручной отрывок, выводим его
        the_excerpt();
    } else {
        // Если нет, выводим первые 8 слов контента (обрезанный the_content)
